...
def init_db():
    ...
    try:
        with conn.cursor() as cursor:
            # Drop all other existing tables
            ...
            # 1. Table for Invoice Output (from Invoice Output.csv)
            cursor.execute("""
            ...
            # 2. Table for Outstanding Output (from Outstanding Output.csv)
            cursor.execute("""
...
if __name__ == "__main__":
    init_db()
    print("MySQL database initialized! All other tables dropped; keeping only 'invoice_output' and 'outstanding_output'.")
