...
const COLUMNS = [
  { col: 'F', label: 'Invoice No' },
  { col: 'R', label: 'Order No' },
  { col: 'C', label: 'Customer Name', minWidth: '200px' },
  { col: 'H', label: 'Catalog No' },
  { col: 'I', label: 'Description', minWidth: '180px' },
  { col: 'K', label: 'Qty', align: 'right' },
  { col: 'N', label: 'Unit Price', align: 'right' },
  { col: 'O', label: 'Invoice Date' },
  { col: 'P', label: 'Net Amount', align: 'right' },
];
    ...
    <div className="page-view animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: '0.25rem 0 0 0' }}>
          Historical invoiced sales records from <code style={{ background: 'var(--bg-hover)', padding: '0.2rem 0.4rem', borderRadius: '4px' }}>Invoice Output.csv</code> (19,046 Records) — Columns shown with Excel-style references (A, B, C...)
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
        <KpiCard
          ...
          subtext="Net Domestic Invoiced Amount (Col P)"
          icon={DollarSign}
      ...
      <div className="glass-card" style={{ padding: '1rem 1.25rem' }}>
        <div style={{ position: 'relative', minWidth: '280px' }}>
          ...
          <input
            type="text"
            placeholder="Search Invoice No (F), Order No (R), Customer (C), Catalog No (H), Description (I)..."
            value={search}
      ...
      <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto', maxHeight: '600px' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.825rem', textAlign: 'left' }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={COLUMNS.length + 1} style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-muted)' }}>Loading Invoice Output records...</td></tr>
                ...
                data.rows.map((row, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td style={{ padding: '0.65rem 1rem', fontWeight: 600 }}>{row.delivery_customer_name || '-'}</td>
                      ...
                      <td style={{ padding: '0.65rem 1rem', whiteSpace: 'nowrap', color: 'var(--text-subtle)' }}>{row.invoice_date ? row.invoice_date.split(' ')[0] : '-'}</td>
                      <td style={{ padding: '0.65rem 1rem', textAlign: 'right', fontWeight: 700, color: 'var(--gsh-red)' }}>{formatCurrency(row.net_dom_amount)}</td>
                    </tr>
                  );
                })
              ) : (
