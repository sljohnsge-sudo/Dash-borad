    ...
    <div className="page-view animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div>
        ...
        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', margin: '0.25rem 0 0 0' }}>
          Outstanding orders and backlog records from <code style={{ background: 'var(--bg-hover)', padding: '0.2rem 0.4rem', borderRadius: '4px' }}>Outstanding Output.csv</code> (170 Records)
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '1rem' }}>
        <KpiCard 
          ...
          subtext="Total Outstanding Order Backlog"
          icon={DollarSign}
          accentColor="#3b82f6"
        />
        <KpiCard 
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.825rem', textAlign: 'left' }}>
            <thead>
              <tr style={{ background: 'var(--bg-hover)', borderBottom: '1px solid var(--border-color)', color: 'var(--text-main)', fontWeight: 700, position: 'sticky', top: 0, zIndex: 10 }}>
                <th style={{ padding: '0.75rem 1rem', whiteSpace: 'nowrap' }}>Customer No</th>
                ...
                <th style={{ padding: '0.75rem 1rem', textAlign: 'right', whiteSpace: 'nowrap' }}>Qty Due</th>
                <th style={{ padding: '0.75rem 1rem', textAlign: 'right', whiteSpace: 'nowrap' }}>Unit Price</th>
                <th style={{ padding: '0.75rem 1rem', whiteSpace: 'nowrap' }}>Delivery Date</th>
                <th style={{ padding: '0.75rem 1rem', textAlign: 'right', whiteSpace: 'nowrap', color: '#3b82f6' }}>Backlog Value</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan="9" style={{ padding: '2.5rem', textAlign: 'center', color: 'var(--text-muted)' }}>
                    Loading Outstanding Output records...
                  </td>
                </tr>
                ...
                data.rows.map((row, idx) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '0.65rem 1rem', fontWeight: 600, color: 'var(--text-main)' }}>{row.customer_name || '-'}</td>
                    ...
                    <td style={{ padding: '0.65rem 1rem', whiteSpace: 'nowrap', color: 'var(--text-subtle)' }}>
                      {row.planned_delivery_date ? row.planned_delivery_date.split(' ')[0] : '-'}
                    </td>
                    <td style={{ padding: '0.65rem 1rem', textAlign: 'right', fontWeight: 700, color: '#3b82f6' }}>
                      {formatCurrency(row.backlog_value_base_curr)}
                    </td>
                  </tr>
                ))
              ) : (
